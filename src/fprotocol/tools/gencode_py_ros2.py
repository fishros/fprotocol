...
def generate_ros2_imports(mappings):
    ...
    for mapping in mappings:
        ...
        if "/" in msg_type:
            ...
            if (
                len(parts) == 3
            ):  # 格式: package/msg/MessageName 或 package/srv/ServiceName
...
class {class_name}(Node):
    def __init__(self, serial_port='/dev/ttyACM0', baud_rate=115200, timeout=0.1):
        ...
        # 设置协议节点
        self.handler.set_self_node(0x0002, self.{file_name.lower()}_proto)
"""
...
"""
    ...
    def {topic_name}_callback(self, msg):
        ...
        if not hasattr(self.{file_name.lower()}_proto, '{protocol_var}'):
            self.get_logger().warning("协议对象不存在字段 {protocol_var}")
            return

        pdata = self.{file_name.lower()}_proto.{protocol_var}
        ...
        self.{file_name.lower()}_proto.write_{protocol_var}(self.handler, FProtocolType.TRANSPORT_DATA, 0xFF)
        self.get_logger().info(f"收到{topic_name}消息，已广播发送到设备")
"""
    else:
        # 没有ROS2映射时，生成示例回调方法
        content += """
...
def generate_python_ros2_code(input_file, output_directory):
    ...
    if mappings:
        print(
            f"已生成ROS2串口集成文件（包含{len(mappings)}个ROS2映射）: {ros2_filename}"
        )
    else:
...
'''
    ...
    # 生成节点类
    node_content, publishers, subscribers = generate_ros2_node_class(
        file_name, mappings, data_list
    )
