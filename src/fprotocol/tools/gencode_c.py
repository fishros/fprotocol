...
def copy_fprotocol_files(output_directory, code_type):
    ...
    if _HAS_PKG_RESOURCES:
        try:
            fprotocol_h_path = pkg_resources.resource_filename(
                "fprotocol", "fprotocol.h"
            )
            fprotocol_c_path = pkg_resources.resource_filename(
                "fprotocol", "fprotocol.c"
            )
...
def generate_c_code(input_file, output_directory, code_type="c"):
    ...
    for row in data_list:
        ...
        if callback_flag == "1":
            h_file_content += f"int16_t callback_{var_name}(uint16_t type, uint8_t from, uint16_t error_code);\n"
...
""".strip()
    ...
    for row in data_list:
        ...
        struct_desc = (
            "NULL"
            if index2struct_desc[index] == "NULL"
            else f"{index2struct_desc[index]}"
        )
        c_file_content += f"""void write_{var_name}(fprotocol_handler *handler,uint8_t node,uint8_t response)\n{{
    fprotocol_write(handler, node, response ? SERVICE_REQUEST_WRITE : TRANSPORT_DATA, {index}, &{var_name}, sizeof({var_name}),{struct_desc});\n}}\n"""
        c_file_content += f"""void read_{var_name}(fprotocol_handler *handler,uint8_t node)\n{{
