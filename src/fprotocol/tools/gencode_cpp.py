...
def gen_field_enum_type(ttype, is_cpp=False):
    if is_cpp:
        mapping = {
            ...
            "uint64_t": "FProtocol::FieldType::UINT64",
            "int8_t": "FProtocol::FieldType::INT8",
            "int16_t": "FProtocol::FieldType::INT16",
            "int32_t": "FProtocol::FieldType::INT32",
            "int64_t": "FProtocol::FieldType::INT64",
            "float": "FProtocol::FieldType::FLOAT",
            "double": "FProtocol::FieldType::DOUBLE",
            "char": "FProtocol::FieldType::CHAR",
            "bool": "FProtocol::FieldType::BOOL",
        }
...
def generate_struct_descriptor(struct_name, fields, is_cpp=False):
    ...
    for idx, (ttype, name, length, size_field) in enumerate(fields):
        ...
        if is_cpp:
            field_defs.append(
                f"        FProtocol::FieldDescriptor({offset}, {enum_type}, {size_field_str}, {array_len})"
            )
        else:
            field_defs.append(
                f"    {{{offset}, {enum_type}, {size_field_str}, {array_len}}}"
            )
...
def generate_c_code(input_file, output_directory, code_type="c"):
    ...
    if is_cpp:
        ...
        for row in data_list:
            if len(row) >= 4:
                index, data_type, var_name, callback_flag = (
                    row[0],
                    row[1],
                    row[2],
                    row[3],
                ...
                if callback_flag == "1":
                    h_file_content += (
                        f"    FProtocol::CallbackFunction {var_name}_callback_;\n"
                    )
        ...
        for row in data_list:
            ...
            h_file_content += (
                f"    {data_type}* get_{var_name}_ptr() {{ return &{var_name}_; }}\n\n"
            )
        ...
        for row in data_list:
            if len(row) >= 4:
                ...
                if callback_flag == "1":
                    ...
                    h_file_content += (
                        f"            return (obj->*method)(type, from, error_code);\n"
                    )
                    h_file_content += f"        }};\n"
                    h_file_content += f"    }}\n"
        ...
        h_file_content += (
            f"    FProtocol::GetIndexInfoFunction getIndexInfoFunction();\n"
        )
        h_file_content += "};\n\n"
    ...
    if is_cpp:
        ...
        for row in data_list:
            ...
            if data_type in [
                ...
                c_file_content += f"    {var_name}_ = 0.0;\n"
            elif data_type == "bool":
                c_file_content += f"    {var_name}_ = false;\n"
            else:
        ...
        for row in data_list:
            index, data_type, var_name = row[0], row[1], row[2]
            struct_desc = f"{data_type}_desc"
            c_file_content += f"""void {file_name}Protocol::write_{var_name}(FProtocol::Handler* handler, uint8_t node, bool response) {{
...
"""
            c_file_content += f"""void {file_name}Protocol::read_{var_name}(FProtocol::Handler* handler, uint8_t node) {{
...
"""
    ...
    if copy_success:
        ...
        print(
            f"代码生成完成，但fprotocol源文件拷贝失败。请手动拷贝fprotocol.{'hpp' if is_cpp else 'h'}和fprotocol.{'cpp' if is_cpp else 'c'}文件到输出目录。"
        )
...
def main():
    ...
    if len(sys.argv) < 3 or len(sys.argv) > 4:
        print(
            "Usage: python gencode_cpp.py <input_file> <output_directory> [code_type]"
        )
    ...
    print(
        f"参数解析: input_file={input_file}, output_directory={output_directory}, code_type={code_type}"
    )
