...
try:
    ...
    def generate_c_code(input_file, output_directory, code_type='c'):
        ...
        if result.returncode != 0:
            raise RuntimeError(f"Generate C failed: {result.stderr}")
    ...
    def generate_cpp_udp_code(input_file, output_directory, code_type='cpp'):
        ...
        if result.returncode != 0:
            raise RuntimeError(f"Generate C++ UDP failed: {result.stderr}")
    ...
    def generate_python_code(input_file, output_directory):
        ...
        if result.returncode != 0:
            raise RuntimeError(f"Generate Python failed: {result.stderr}")
    ...
    def generate_python_ros2_code(input_file, output_directory):
        ...
        if result.returncode != 0:
            raise RuntimeError(f"Generate Python ROS2 failed: {result.stderr}")
    ...
    def generate_python_ros2_udp_code(input_file, output_directory):
        ...
        if result.returncode != 0:
            raise RuntimeError(f"Generate Python ROS2 UDP failed: {result.stderr}")
...
示例用法:
  ...
  fprotocol_tool gencode example.fproto --type=py --out_dir=. # 生成Python代码在当前目录
    ...
    gencode_parser.add_argument('--type', choices=['c', 'cpp_c', 'cpp', 'cpp_udp', 'py', 'py_ros2_serial', 'py_ros2_udp'], required=True,
...
def handle_gencode(args):
    ...
    try:
        if args.type == 'c':
            ...
            print(f"Generated C code to: {out_dir}")
        elif args.type == 'cpp_c':
            generate_c_code(str(proto_file), str(out_dir), 'cpp')
            print(f"Generated C-style C++ code to: {out_dir}")
        elif args.type == 'cpp':
