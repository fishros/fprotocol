...
void Handler::tick() {
    ...
    if (frame_->getRecvSize() >= 4 && frame_->getRecvSize() < frame_header_size) {
        ...
        if (frame_->getRecvSize() != frame_header_size) {
            ...
            if (header.to == self_node_id_ || header.to == BROADCAST_NODE_ID) {
                switch (static_cast<ProtocolType>(header.type)) {
                case ProtocolType::HEART_PING:
...
#endif
        if (static_cast<ProtocolType>(header.type) == ProtocolType::HEART_PING) {
            if (heart_ping_callback_) {
                heart_ping_callback_(header.from);
            }
            write(header.from, ProtocolType::HEART_PONG, 0, nullptr, 0);
            return;
        }
