    ...
    def parse(self, data: bytes):
        ...
        for field in self.fields:
            ...
            if field_name.startswith("_"):
                self.values[field_name] = struct.unpack(
                    fmt, data[index : index + field_size]
                )[0]
                ...
                if next_array_size:
                    fmt_without_num = fmt[
                        -1
                    ]  # Get the format type (like 's', 'H', etc.)
                    ...
                    unpacked = struct.unpack(
                        dynamic_fmt,
                        data[index : index + next_array_size * fmt_signle_size],
                    )
                    ...
                    logger.debug(
                        f"Unpacked dynamic field {field_name}: {self.values[field_name]}"
                    )
...
class BaseValue:
    ...
    def parse(self, data: bytes):
        ...
        self._value = struct.unpack(
            self._format, data[: struct.calcsize(self._format)]
        )[0]
...
class CircularBuffer:
    ...
    def put(self, data):
        logger.debug(
            f"Buffer size: {self.csize}, Current count: {self.count}, Data length: {len(data)}"
        )
...
class FProtocol:
    ...
    def _is_target_node(self, to_node):
        """仅处理发给自己的数据或广播数据。"""
        return to_node == self.self_node_id or to_node == 0xFF
    ...
    def send_time_sync(self, target_node=None):
        ...
        self.fprotocol_write(
            target_node, FProtocolType.TIME_SYNC_REQ, 0, payload, len(payload)
        )
        return t0_ms
    ...
    def tick(self):
        ...
        if self.frame["recv_size"] >= 4 and self.frame["recv_size"] < 11:
            ...
            if self._is_target_node(header.to):
                if header.type == FProtocolType.HEART_PING:
                    self.frame["data_size"] = 0
                    self.frame["fdata"] = None
                    logger.debug(
                        f"Recv data from node {self.frame['header'].from_node} data_size={self.frame['header'].data_size}"
                    )
                else:
                    ...
                    elif header.type == FProtocolType.TRANSPORT_DATA or header.type in (
                        FProtocolType.TIME_SYNC_REQ,
                        FProtocolType.TIME_SYNC_RES,
                    ):
                        self.frame["data_size"] = self.frame["header"].data_size
                    else:
                        self._reset_frame_state()  # 重新接收
            # print(self.frame['recv_size'],self.frame['data_size'])
            else:
                self._reset_frame_state()

        if self.frame["recv_size"] >= 11 and self.frame["recv_size"] < (
            11 + self.frame["data_size"]
        ):
            ...
            self.frame["recv_size"] = len(self.frame["data"])
            logger.debug(
                f"Recv data from node {self.frame['header'].from_node} data_size={self.frame['data_size']}"
            )
            ...
            if calculated_checksum == received_checksum:
                if self.frame["header"].type == FProtocolType.SERVICE_RESPONSE_ERROR:
                    self.frame["error_code"] = (
                        self.frame["data"][11] << 8 | self.frame["data"][12]
                    )
    ...
    def process_frame(self, frame):
        ...
        elif header.type == FProtocolType.TIME_SYNC_REQ:
        ...
        # 时间同步：收到对端时间
        elif header.type == FProtocolType.TIME_SYNC_RES:
            payload = bytes(frame["data"][11 : 11 + header.data_size])
        ...
        # 处理其他类型的消息
            ...
            if (
                header.type == FProtocolType.SERVICE_RESPONSE_READ
                or header.type == FProtocolType.TRANSPORT_DATA
            ):
                if self.self_node_proto is not None:
                    fdata = self.self_node_proto.get_index_data(
                        self.frame["header"].index
                    )
    ...
    def fprotocol_write(self, to, type, index, data, data_size=0):
        from_node = self.self_node_id if self.self_node_id is not None else 0
        frame = []
