...
static size_t fprotocol_field_type_size(uint8_t type)
{
    switch (type)
    {
    case TYPE_INT8:
...
size_t fprotocol_pack_struct(uint8_t *buffer, const void *data, const StructDescriptor *desc)
{
    ...
    for (int i = 0; i < desc->field_count; ++i)
    {
        ...
        if (f.size_field >= 0)
        {
            ...
            #endif
            len *= fprotocol_field_type_size(f.type);
        }
        else if (f.array_len > 0)
        {
            // 固定长度数组，需要乘以元素大小
            size_t type_size = fprotocol_field_type_size(f.type);
            len = type_size > 0 ? (f.array_len * type_size) : f.array_len;
        }
        else
        {
            // 单值字段
