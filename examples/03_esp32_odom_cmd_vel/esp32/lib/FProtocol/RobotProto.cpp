...
{
    fprotocol_write(handler, node, SERVICE_REQUEST_READ, 0x0002, &odom, sizeof(odom),&float3d_t_desc);
}
...
{
    fprotocol_write(handler, node, SERVICE_REQUEST_READ, 0x0003, &cmd_vel, sizeof(cmd_vel),&float3d_t_desc);
}
